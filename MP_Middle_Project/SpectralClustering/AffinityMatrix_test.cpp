...
double _quickSelection(double* distances, double* pivot_left, double* pivot_right, int size, int k) {
    ...
    if(k <= pivot_left_count) {
        return _quickSelection(pivot_left, pivot_left, pivot_right, pivot_left_count, k);
    } else if(k >= pivot_left_count + 1 && k <= pivot_left_count + same) {
        return pivot; 
    } else {
        return _quickSelection(pivot_right, pivot_left, pivot_right, pivot_right_count, k - pivot_left_count - same);
    }
}

double** _generateAffinityMatrix(Point* points, int point_count) {
    ...
    for(int p1 = 0; p1 < point_count; p1++) {
        ...
        deltas[p1] = _quickSelection(partial_distance, pivot_left, pivot_right, point_count, SIGMA_DIMENSION);
    }
...
double** __generateAffinityMatrix_parallel(Point* points, int point_count) {
    double** result = new double*[point_count];
    double** distance = new double*[point_count];
    double* deltas = new double[point_count];
    ...
    #pragma omp parallel num_threads(NUM_THREADS) shared(result, distance, deltas)
    {
        ...
        for(int p1 = 0; p1 < point_count; p1++) {
            ...
            distance[p1] = partial_distance;
            #if !USE_V2_PARALLEL
            deltas[p1] = _quickSelection(partial_distance, local_pivot_left, local_pivot_right, point_count, SIGMA_DIMENSION);
            #endif
        }
        ...
        for(int p1 = 0; p1 < point_count; p1++) {
            ...
            deltas[p1] = _quickSelection(distance[p1], local_pivot_left, local_pivot_right, point_count, SIGMA_DIMENSION);
        }
        #endif
...
double** _generateAffinityMatrix_V2(Point* points, int point_count) {
    double** result = new double*[point_count];
    double** distance = new double*[point_count];
    double* deltas = new double[point_count];
...
double** __generateAffinityMatrix_parallel_V2(Point* points, int point_count) {
    double** result = new double*[point_count];
    double** distance = new double*[point_count];
    double* deltas = new double[point_count];
...
//int main() 
{
    ...
    DS_timer timer(4);
	timer.setTimerName(0, (char*)"Serial V1");
	timer.setTimerName(1, (char*)"Parallel V1");
	timer.setTimerName(2, (char*)"Serial V2");
	timer.setTimerName(3, (char*)"Parallel V2");
    for(int i = 0; i < TEST_DATA_COUNT; i++) {
        points[i] = Point(GenDouble, GenDouble);
    }

    timer.onTimer(0);
    double** result = _generateAffinityMatrix(points, TEST_DATA_COUNT);
    timer.offTimer(0);

    timer.onTimer(1);
    double** result2 = __generateAffinityMatrix_parallel(points, TEST_DATA_COUNT);
    timer.offTimer(1);

    timer.onTimer(2);
    double** resul3 = _generateAffinityMatrix_V2(points, TEST_DATA_COUNT);
    timer.offTimer(2);

    timer.onTimer(3);
    double** result4 = __generateAffinityMatrix_parallel_V2(points, TEST_DATA_COUNT);
    timer.offTimer(3);
