...
/// @license  CC0-1.0
/// @link     https://github.com/jewettaij/Jacobi_pd
...
///@brief Find the off-diagonal index in row i whose absolute value is largest
    ...
    public:
        ...
        JacobiMulti(const JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>& source);
        JacobiMulti(JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>&& other);
        void swap(JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>& other) noexcept;
        JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>& operator = (JacobiMulti<Scalar, Vector, Matrix, ConstMatrix> source);
    ...
    int JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>::
        Diagonalize(ConstMatrix mat,    // the matrix you wish to diagonalize (size n)
            Vector eval,        // store the eigenvalues here
            Matrix evec,        // store the eigenvectors here (in rows)
            ...
            int max_num_sweeps) // limit the number of iterations ("sweeps")
    {
        ...
        //#pragma omp parallel for num_threads(NUM_THREADS) lastprivate(n_iters)
        //for (int k = 1; k <= max_num_iters; k++) {
    ...
    template<typename Scalar, typename Vector, typename Matrix, typename ConstMatrix>
    void JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>::
    ...
    ///         angle ��.  This function assumes that c=cos(��). s=sin(��), t=tan(��)
    ...
    template<typename Scalar, typename Vector, typename Matrix, typename ConstMatrix>
    JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>::
        JacobiMulti(const JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>& source)
    {
        Init();
        SetSize(source.n);
    ...
    template<typename Scalar, typename Vector, typename Matrix, typename ConstMatrix>
    void JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>::
        swap(JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>& other) noexcept {
        std::swap(n, other.n);
    ...
    JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>::
        JacobiMulti(JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>&& other)
        : JacobiMulti()  // delegate to the default constructor (which invokes Init())
    {
        this->swap(other);
    }
    ...
    JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>&
        JacobiMulti<Scalar, Vector, Matrix, ConstMatrix>::
        operator = (JacobiMulti<Scalar, Vector, Matrix, ConstMatrix> source) {
        this->swap(source);
        return *this;
    }
