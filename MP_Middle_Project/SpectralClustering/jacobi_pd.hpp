...
namespace jacobi_pd {
    ...
    private:
        ...
        ///        rotation in the i,j plane by an angle (еш) causing M[i][j]=0.
        ///        The resulting parameters will be stored in this->c, this->s, and
        ///        this->t (which store cos(еш), sin(еш), and tan(еш), respectively).
...
///@brief Find the off-diagonal index in row i whose absolute value is largest
    ...
    /// @brief Calculate the components of a rotation matrix which performs a
    ///        rotation in the i,j plane by an angle (еш) that (when multiplied on
    ///        both sides) will zero the ij'th element of M, so that afterwards
    ///        M[i][j] = 0.  The results will be stored in c, s, and t
    ///        (which store cos(еш), sin(еш), and tan(еш), respectively).
    ...
    ///         angle еш.  This function assumes that c=cos(еш). s=sin(еш), t=tan(еш)
    ...
    ///
    /// verbatim
    ///   M'_uv =  е╥_w  е╥_z   R_wu * M_wz * R_zv
    /// endverbatim
    ///
    ...
    ///       This matrix performs a rotation in the i,j plane by angle еш  (where
    ///       the arguments "s" and "c" refer to cos(еш) and sin(еш), respectively).
    /// @verbatim
    ///   E'_uv = е╥_w  R_wu * E_wv
    /// @endverbatim
    ...
    void Jacobi<Scalar, Vector, Matrix, ConstMatrix>::
        ApplyRotLeft(Matrix E,  // matrix
            ...
            int j)     // column index
    {
        // Recall that c = cos(еш) and s = sin(еш)
        for (int v = 0; v < n; v++) {
